...
##### 3. 写一个redis配置类  
 详情请见：https://github.com/higminteam/practice/blob/master/src/main/java/com/practice/spring/springbootAndRedis/IConfigurationForRedis  

##### 4. 利用redis 实现分布式锁
思路：这个主要利用redis的setnx命令进行，setnx："set if not exists"就是如果不存在则成功设置缓存同时返回1，否则返回0 。  
 详情请见：https://github.com/higminteam/practice/blob/master/src/main/java/com/practice/spring/springbootAndRedis/distributedLock/RedisDistributedLock.java  
